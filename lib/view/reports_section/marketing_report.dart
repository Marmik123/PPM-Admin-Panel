...
class MarketingReport extends StatelessWidget {
...
/*      DatatableHeader(
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("Marketing Person Report"),
        actions: [
          IconButton(
          ...
          children: [
            ...
            Obx(
              () =>
                  ...
                  :*/
                  mCtrl.isLoading.value
                      ? buildLoader()
                      : (mCtrl.marketingReport()?.isNotEmpty ?? false)
                          ? Expanded(
                              child: ListView(
                                shrinkWrap: true,
                                children: [
                                  ResponsiveDatatable(
                                    headers: header,
                                    autoHeight: true,
                                    isLoading: mCtrl.isLoading.value,
                                    ...
                                    source: List.generate(
                                        mCtrl.marketingReport.isNotEmpty
                                            ? mCtrl.marketingReport?.length
                                            : null,
                                        (index) => {
                                              'id': index + 1,
                                              'index': index,
                                              'object':
                                                  mCtrl.marketingReport[index],
                                              'name':
                                                  mCtrl.marketingReport()[index]
                                                          ['markPName'] ??
                                                      "-",
